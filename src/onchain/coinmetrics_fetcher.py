"""CoinMetrics Community API fetcher for BTC on-chain metrics (Experiment 6).

Pulls the daily active-address count (``AdrActCnt``) for BTC from the free
CoinMetrics Community v4 endpoint and caches it locally as parquet so repeated
pipeline runs never re-hit the network.
...
Usage:
    python -m src.onchain.coinmetrics_fetcher
"""
...
CACHE_PATH = _REPO_ROOT / "data" / "onchain" / "btc_adr_act_cnt.parquet"
...
def fetch_adr_act_cnt(start_time: str = START_TIME) -> pd.DataFrame:
    """Fetch the full daily ``AdrActCnt`` series for BTC from CoinMetrics.
    ...
    if not records:
        raise RuntimeError("CoinMetrics returned no data for AdrActCnt.")
...
def load_adr_act_cnt(
    cache_path: Path = CACHE_PATH, refresh: bool = False
) -> pd.DataFrame:
    """Return the cached AdrActCnt series, fetching and caching on first use.
...
def main() -> None:
    df = load_adr_act_cnt()
    print(f"[coinmetrics_fetcher] cached at: {CACHE_PATH}")
    print(f"[coinmetrics_fetcher] rows: {len(df)}, "
