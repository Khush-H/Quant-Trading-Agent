"""Experiment 8 runner: BTC/USDT 1h walk-forward with Coinbase-premium features.

Pre-registered windows (hard-coded; not flags, so they cannot drift):

* TUNING : 2019-01-01 <= candle open ts < 2021-01-01   (the default)
* HOLDOUT: 2021-01-01 <= candle open ts < 2023-01-01   (LOCKED, hard cap)

The holdout end is a HARD CAP at 2023-01-01. The holdout window refuses to
run without ``--confirm-holdout``, which must only ever be passed after the
explicit "RUN HOLDOUT" instruction. Nothing in the tuning path reads a single
bar or premium value dated at/after 2021-01-01: the premium frame is
truncated strictly below the window end before features are built (on top of
the T-inclusive PIT rule proven by
``tests/test_coinbase_premium_pit_leakage.py``).

Everything else is the unchanged pipeline: causal OHLCV features (+3 premium
columns), Flat/Long labels (N=1, 24bps hurdle), walk-forward XGBoost
(5 splits, embargo = label horizon), event-driven spot backtest (10bps taker
+ 2bps slippage per side, next-bar-open execution, 20% fixed-fractional,
long-only). Threshold fixed at the 0.50 default — nothing is tuned on
results.

Usage:
    python -m scripts.run_premium_backtest                  # tuning window
    python -m scripts.run_premium_backtest --window holdout --confirm-holdout
"""
...
# candle OPEN ts.
TUNING_START_MS = 1546300800000   # 2019-01-01
TUNING_END_MS = 1609459200000     # 2021-01-01
HOLDOUT_END_HARDCAP_MS = 1672531200000  # 2023-01-01 — hard cap, never exceeded
...
def _load_capped_ohlcv(db, start_ms: int, end_ms: int) -> pd.DataFrame:
    ...
    if ohlcv.empty:
        raise SystemExit(f"No candles inside the {start_ms}..{end_ms} window.")
    # Belt-and-braces: prove the cap held before anything downstream runs.
    assert int(ohlcv.index.max()) < end_ms, "window cap violated"
    assert int(ohlcv.index.max()) < HOLDOUT_END_HARDCAP_MS, "2023-01-01 hard cap violated"
    return ohlcv


def _load_capped_premium(end_ms: int) -> pd.DataFrame:
    from src.onchain.coinbase_premium_fetcher import load_premium

    prem = load_premium()  # parquet cache; no network on reruns
    # The T-inclusive PIT rule inside the feature builder is the per-row
    # guarantee; truncating the frame strictly below the window end is the
    # extra fence so out-of-window data is not even present in memory.
    cutoff = pd.Timestamp(end_ms, unit="ms")
    return prem.loc[prem["timestamp_utc"] < cutoff].reset_index(drop=True)
...
def run(window: str) -> dict:
    ...
    from src.onchain.premium_features import PREMIUM_FEATURE_COLUMNS
    ...
    prem = _load_capped_premium(end_ms)

    features = build_features(ohlcv, premium=prem)
    expected_cols = list(FEATURE_COLUMNS) + list(PREMIUM_FEATURE_COLUMNS)
    ...
    def _d(ts_ms: int) -> str:
        return str(pd.Timestamp(int(ts_ms), unit="ms"))

    print("\n" + "=" * 68)
    print(f"EXPERIMENT 8 — {SYMBOL} {TIMEFRAME} + Coinbase premium "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Experiment 8 walk-forward run.")
