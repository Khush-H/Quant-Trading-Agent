"""Experiment 9 runner: LINK/USDT 1d walk-forward with Coinbase-premium features.

Pre-registered windows (hard-coded; not flags, so they cannot drift):

* TUNING : 2020-10-01 <= candle open ts < 2024-01-01   (the default)
* HOLDOUT: 2024-01-01 <= candle open ts < 2025-06-01   (LOCKED, hard cap)

The holdout end is a HARD CAP at 2025-06-01. The holdout window refuses to
...
bar or premium value dated at/after 2024-01-01: the premium frame is
truncated strictly below the window end before features are built (on top of
the T-inclusive PIT rule proven by
``tests/test_link_premium_pit_leakage.py``).

Everything else is the unchanged pipeline: causal OHLCV features (+3 premium
columns at DAILY resolution — 168d z-window, 24d momentum, same bar counts
as Experiment 8), Flat/Long labels (N=1, 24bps hurdle), walk-forward XGBoost
...
Usage:
    python -m scripts.run_link_premium_backtest                  # tuning window
    python -m scripts.run_link_premium_backtest --window holdout --confirm-holdout
"""
...
# candle OPEN ts.
TUNING_START_MS = 1601510400000   # 2020-10-01
TUNING_END_MS = 1704067200000     # 2024-01-01
...
def _load_capped_premium(end_ms: int) -> pd.DataFrame:
    ...
    prem = load_premium(asset=PREMIUM_ASSET, timeframe=TIMEFRAME)
    # parquet cache; no network on reruns. The T-inclusive PIT rule inside the
    # feature builder is the per-row guarantee; truncating the frame strictly
    # below the window end is the extra fence so out-of-window data is not
    # even present in memory.
...
def run(window: str) -> dict:
    ...
    print("\n" + "=" * 68)
    print(f"EXPERIMENT 9 — {SYMBOL} {TIMEFRAME} + Coinbase premium "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Experiment 9 walk-forward run.")
