"""REQUIRED leakage test: Coinbase-premium T-inclusive PIT rule (Experiment 8).

The premium analogue of the on-chain D-1 proof. The pre-registered rule:

    A feature row for the 1h bar at T uses ONLY coinbase_premium values dated
    T or earlier. The premium at T comes from both exchanges' CLOSED bars at
    T, so T itself is legitimately usable; what must be proven is that the
    rolling windows (168h z-score, 24h momentum) never reach past T.
...
# Sized so >= 100 rows sit past the 168h z-warmup + 24h momentum lag: 500
# hourly candles, premium covering 200 hours of lead and 30 hours of trail
...
def _make_ohlcv(seed: int = 17) -> pd.DataFrame:
    ...
    ts = _FIRST_BAR.value // 10**6 + np.arange(_N_BARS, dtype=np.int64) * _HOUR_MS
    close = 50_000.0 * np.exp(np.cumsum(rng.normal(0.0, 0.005, _N_BARS)))
    spread = np.abs(rng.normal(0.0, 0.002, _N_BARS))
    return pd.DataFrame(
        {
            "open": close * (1 + rng.normal(0.0, 0.001, _N_BARS)),
            "high": close * (1 + spread),
            "low": close * (1 - spread),
            "close": close,
            "volume": rng.uniform(10, 500, _N_BARS),
        },
...
def _make_premium(seed: int = 23) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    times = pd.date_range(
        _FIRST_BAR - _PREMIUM_LEAD_H * _HOUR,
        _FIRST_BAR + (_N_BARS - 1 + _PREMIUM_TRAIL_H) * _HOUR,
        freq="h",
    )
...
def test_features_match_hand_computation_from_T_or_earlier():
    ...
    ever handed premium values dated T or earlier."""
    ohlcv, premium = _make_ohlcv(), _make_premium()
    ...
    for row_ts in _sampled_rows(ohlcv):
        ...
        window = by_time.loc[t - (PREMIUM_Z_WINDOW - 1) * _HOUR: t]
        ...
        denom = by_time.loc[t - PREMIUM_MOM_LAG * _HOUR]
...
def test_injected_T_plus_1_value_is_caught_by_the_rule_check():
    ...
    def leaky_builder(ohlcv: pd.DataFrame, premium: pd.DataFrame) -> pd.DataFrame:
        # Re-dating every value one hour EARLIER makes the T sampling inside
        # add_premium_features read the true T+1 value: exactly the
        # "use premium[T+1] for row T" leak.
        return add_premium_features(
            ohlcv, premium.assign(timestamp_utc=premium["timestamp_utc"] - _HOUR)
        )
